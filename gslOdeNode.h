...
namespace conedy
{
	...
	//! Basisklasse fÃ¼r Nodes, die mit der GSL-integriert werden.
	class gslOdeNode : public containerNode<baseType,3>, private globals
	{
		private:
			...
			// Hilfsfunktionen fÃ¼r die GSL:
			//! Endzeit der Integration
			//! Zeiger auf die Art der Stufenfunktion (hier kann die Stufenfunktion gewÃ¤hlt werden)
			static const gsl_odeiv2_step_type * stepType;

			//! Zeiger auf den Speicher fÃ¼r die Stufenfunktion
			static gsl_odeiv2_step * gslStep;

			//! Zeiger auf die Kontrollfunktion (Ã¼berprÃ¼ft Fehler der Stufenfunktion)
			static gsl_odeiv2_control * gslControl;

			//! Zeiger auf die Evolutionsfuktion (diese vereint die Stufenfunktion mit der Kontrollfunktion)
			static gsl_odeiv2_evolve * gslEvolve;

			//! Datentyp fÃ¼r das ODE-System
...
/*					else if (theStepType == "gsl_odeiv2_step_gear1")
			...
			//! Kopieren der Temp-ZustÃ¤nde in den Zustand nach erfolgter Integration
			//			virtual void swap()
			//			{
			//				this->state = this->x[0];
			//
